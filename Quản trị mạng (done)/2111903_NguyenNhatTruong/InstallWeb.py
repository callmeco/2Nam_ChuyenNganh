...
class InstallWeb:
    def __init__(self,root):
        ...
        createP_H = LabelFrame(self.root,text="Install Web Service",bg='white',bd=2,relief=RIDGE)
        createP_H.place(x=10,y=20,width=1000,height=550)
        heading_lb=Label(createP_H,text="Install Web Service",bg="white",font="40").place(x=400,y=100)
    ...
    def installService(self):
        command = "powershell.exe Install-WindowsFeature Web-Server -IncludeManagementTools"
        try:
            sendCommand(command)
            os.system(command)
